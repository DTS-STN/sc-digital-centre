...
describe('MapSummary', () => {
  ...
  describe('Map money types', () => {
    const sums = [
      ...
      { type: SummaryTypes.Payment, value: '2000.00' },
    ]
    ...
    it('should map Payment', () => {
      const sut = result[2]
      expect(sut).toBeDefined()
      expect(sut.title).toBe('Payment')
      expect(sut.value).toBe('$2,000.00')
      expect(sut.valueStyle).toBe('text-4xl')
    })
  })
  ...
  describe('Map date types', () => {
    const sums = [
      ...
      { type: SummaryTypes.PaymentDate, value: '2022-06-21' },
    ]
    const result = MapSummary(sums, enT, 'en')
    ...
    it('should map PaymentDate', () => {
      const sut = result[8]
      expect(sut).toBeDefined()
      expect(sut.title).toBe('Payment Date')
      expect(sut.status).toBeUndefined()
      expect(sut.value).toBe('June 21, 2022')
      expect(sut.link).toBe('/')
      expect(sut.linkText).toBe('View my payments')
