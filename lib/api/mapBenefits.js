...
import { formatDate, newNormalizedDate } from '../Utils'
...
export function MapOASCard(oasBenefitData) {
  ...
  switch (status) {
    ...
    case StatusCodes.applicationReceived:
      const lastStatusDate = newNormalizedDate(oasBenefitData.lastStatusDate)
...
export function MapGISCard(gisBenefitData) {
  ...
  switch (status) {
    ...
    case StatusCodes.applicationReceived:
      const lastStatusDate = newNormalizedDate(gisBenefitData.lastStatusDate)
      lastStatusDate.setDate(lastStatusDate.getDate() + 14)
      summaries = [
        FormatSummary(SummaryTypes.RequestedBenefit, type),
