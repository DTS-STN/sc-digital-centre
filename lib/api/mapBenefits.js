...
import { MapBenefit, FormatSummary } from './mapBenefitsHelper'
...
export function MapEICard(eiBenefitData) {
  ...
  let lastMessage
  if (eiBenefitData.messageData) {
    lastMessage =
      eiBenefitData.messageData[eiBenefitData.messageData.length - 1]
  }
  let summaries
  ...
  switch (status) {
    ...
    case StatusCodes.benefitUpdate:
      summaries = [
        ...
        FormatSummary(SummaryTypes.LatestStatusMessage, {
          en: lastMessage?.messageDataValueEn,
          fr: lastMessage?.messageDataValueFr,
        }),
      ]
      break
...
export function MapCPPCard(cppBenefitData) {
  ...
  const type = CPPTypes[cppBenefitData.benefitCode]
  ...
  switch (status) {
    case StatusCodes.inPayment:
      let nextPayment = newNormalizedDate(cppBenefitData.paymentDate)
      ...
      summaries = [
        ...
        FormatSummary(SummaryTypes.LastPaymentDate, cppBenefitData.paymentDate),
