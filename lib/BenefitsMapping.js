...
export function MapEICard(eiBenefitData) {
  ...
  switch (status) {
    ...
    case StatusCodes.applicationReceived:
      const lastPaymentDate = new Date(eiBenefitData.lastPaymentDate)
      summaries = [
        FormatSummary(SummaryTypes.RequestedBenefit, type),
        FormatSummary(SummaryTypes.LatestStatus, eiBenefitData.lastStatusDate),
        FormatSummary(
    ...
    case StatusCodes.benefitExhausted:
      summaries = [
...
export function MapCPPDCard(cppdBenefitData) {
  ...
  const status = CPPDStatus.find(
    (s) => s.value == cppdBenefitData.benefitStatus
  ).status
  let summaries
  ...
  const benefit = MapBenefit(ProgramCodes.CPPD, status, type, summaries)
  return benefit
}
